...
function LostAndFoundListEdit(data, count, page, classname) {
	...
	this.saveMarkedStatus = function(data) {
		...
		ajaxRequest.openPost(
				'index.php?page=AdminToolsLostAndFoundAction&pagename='
						+ this.page + '&classname=' + this.classname
						+ '&packageID=' + PACKAGE_ID + SID_ARG_2ND, data);
	}
	...
	this.getEditOptions = function(id) {
		...
		var markedStatus = item ? item.isMarked : false;
		options[i] = new Object();
		options[i]['function'] = 'itemListEdit.parentObject.markItem('
	...
	this.remove = function(id) {
		if (confirm(language['wcf.acp.admintools.delete.sure'])) {
			document.location.href = fixURL('index.php?page=AdminToolsLostAndFoundAction&pagename='
					+ this.page
					+ '&classname='
					+ this.classname
	...
	this.unmarkAll = function() {
		var ajaxRequest = new AjaxRequest();
		ajaxRequest
				.openGet('index.php?page=AdminToolsLostAndFoundAction&pagename='
						+ this.page
						+ '&classname='
						+ this.classname
